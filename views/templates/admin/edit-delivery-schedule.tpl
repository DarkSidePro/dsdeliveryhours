...
<div class="panel">
	<h3><i class="icon icon-truck"></i> {l s='DS: Delivery hours - edit schedule' mod='dsdeliveryhours'}</h3>
	<form id='addDeliverySchema' class='form-horizontal' method='POST'>
		<input type='hidden' name='editDeliverySchema' value='{$data.id_deliveryhoursrules|strip_tags:"UTF-8"|escape:"htmlall":"UTF-8"}'>
						...
						<tr>
							<td>                            
								<select class='form-control' name='dayWeek'>
									...
									{foreach $days as $day}
										<option value='{$day.id|strip_tags:"UTF-8"|escape:"htmlall":"UTF-8"}' {if $day.id == $data.rules_day_ID}selected{/if}>{$day.day|strip_tags:"UTF-8"|escape:"htmlall":"UTF-8"}</option>
									{/foreach}
								</select>
							</td>
							<td>
								<select class='form-control' name='hourStart'>
									...
									{foreach $hours as $hour}
										<option value='{$hour.id_deliveryhoursinterval|strip_tags:"UTF-8"|escape:"htmlall":"UTF-8"}' {if $hour.id_deliveryhoursinterval == $data.rules_interval_start}selected{/if}>{$hour.interval_time|strip_tags:"UTF-8"|escape:"htmlall":"UTF-8"|truncate:5:""}</option>
									{/foreach}									
								</select>
							</td>
							<td>
								<select class='form-control' name='hourStop'>
									...
									{foreach $hours as $hour}
										<option value='{$hour.id_deliveryhoursinterval|strip_tags:"UTF-8"|escape:"htmlall":"UTF-8"}' {if $hour.id_deliveryhoursinterval == $data.rules_interval_stop}selected{/if}>{$hour.interval_time|strip_tags:"UTF-8"|escape:"htmlall":"UTF-8"|truncate:5:""}</option>
									{/foreach}	
								</select>
							</td>
							<td>
								<input type='date' class='form-control' name='dateStart' value='{$data.rules_start|strip_tags:"UTF-8"|escape:"htmlall":"UTF-8"}'>
							</td>
							<td>
								<input type='date' class='form-control' name='dateStop' value='{$data.rules_stop|strip_tags:"UTF-8"|escape:"htmlall":"UTF-8"}'>
							</td>						
							<td>
								<select class='form-control' id='selectCarrier' name='selectCarrier'>
									...
									{foreach $carriers as $carrier}						
										<option value='{$carrier.id_deliveryhoursshipper|strip_tags:"UTF-8"|escape:"htmlall":"UTF-8"}' {if $data.id_deliveryhoursshipper == $carrier.id_deliveryhoursshipper}selected{/if}>{$carrier.shipper_name|strip_tags:"UTF-8"|escape:"htmlall":"UTF-8"}</option>
									{/foreach}
								</select>
							</td>				
							<td>
								<input type='number' class='form-control' name='maxOrdersPerInterval' min='0' value='{$data.rules_quantity|strip_tags:"UTF-8"|escape:"htmlall":"UTF-8"}'>
							</td>								
						</tr>
					</tbody>
				</table>
